...
const HomePage = () => {
  ...
  const [jobs, setJobs] = useState([
    {
      ...
      company: "ABC Corp",
      location: "Hà Nội",
      description: "Developing and maintaining frontend UI.",
    },
    {
      id: 2,
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <div className="flex flex-row gap-4 fixed top-4 right-5 z-10 bg-white ">
        <button
          onClick={() => {
            console.log("Đăng ký");
            router.push("/login");
        ...
        <button className=" px-6 py-3 bg-yellow-400 text-black rounded-md hover:bg-yellow-500">
          Đăng ký
        </button>
      </div>
      ...
      <header className="header text-center mb-8">
        <h1 className="title text-3xl font-bold text-gray-800">
          Tìm kiếm công việc cho sinh viên
        </h1>
        <input
          type="text"
      ...
      <section className="job-list grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredJobs.length > 0 ? (
          filteredJobs.map((job) => (
            <div
              key={job.id}
              className="job-card p-6 bg-white rounded-lg shadow-md hover:shadow-lg"
            >
              <h3 className="job-title text-xl font-semibold text-gray-800">
                {job.title}
              </h3>
              <p className="job-company text-sm text-gray-600">{job.company}</p>
              <p className="job-location text-sm text-gray-500">
                {job.location}
              </p>
              <p className="job-description text-sm text-gray-700 mt-2">
                {job.description}
              </p>
          ...
          <p className="no-jobs text-center text-lg text-gray-500">
